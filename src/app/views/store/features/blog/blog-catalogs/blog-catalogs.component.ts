...
export class BlogCatalogsComponent implements OnInit {
  ...
  ngOnInit() {
    if(!this.commonService.desktop_device) {
      ...
      this.commonService.secondary_header = "Blog Catalogs";
    }
    ...
    this.fApi.BLOG_CATALOG_LIST().subscribe(result => {
      if (result.status) {
  ...
  openPopup(type, data, modalName) {
    ...
    if (type == 'add') {
      ...
      this.modalService.open(modalName, { size: 'lg', windowClass: 'scroll-modal-lg', scrollable: true });
      this.fApi.BLOG_CATALOG_DETAILS(data._id).subscribe((result)=> {
        if(result.status) {
  ...
  onSubmit() {
    ...
    if (this.catForm.form_type == 'add') {
      this.fApi.ADD_BLOG_CATALOG(this.catForm).subscribe(result => {
      ...
      this.fApi.UPDATE_BLOG_CATALOG(this.catForm).subscribe(result => {
        this.catForm.submit = false;
        if (result.status) {
          document.getElementById('closeModal').click();
          this.ngOnInit();
  ...
  onDelete() {
    this.catForm.submit = true;
    this.fApi.DELETE_BLOG_CATALOG(this.catForm).subscribe(result => {
      this.catForm.submit = false;
      if (result.status) {
        document.getElementById('closeModal').click();
        this.ngOnInit();
