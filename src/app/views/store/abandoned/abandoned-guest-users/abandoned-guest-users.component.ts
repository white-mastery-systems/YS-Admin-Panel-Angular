        ...
        this.list.forEach((element, index) => {
          try {
            ...
            if(element.address_list?.length) {
              let addrData = element.address_list[0];
              element.name = addrData.name;
              element.mobile = addrData.mobile;
              if(element.mobile.charAt(0) === '0') element.mobile = element.mobile.substring(1);
              if(addrData.dial_code) element.mobile = addrData.dial_code+" "+element.mobile;
              element.city = addrData.city || "-";
              element.country = addrData.country || "-";
